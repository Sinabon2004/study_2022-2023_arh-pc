...
2. Введём в этот файл программу из лабораторной(Листинг 10.1). (рис. [-@fig:001], [-@fig:002])
...
3. Изменим текст программы, по задаче из лабораторной, чтобы вычислялось f(g(x)), где f(x) = 2x + 7, g(x) = 3x − 1(рис. [-@fig:003], [-@fig:004])
...
4. Создадим lab10-2.asm с текстом программы из Листинга 10.2:
(рис. [-@fig:005])

![Файл lab10-2.asm](image/05.png){ #fig:005 width=70%, height=70% }

Проверьте работу программы, запустив ее в оболочке GDB:(рис. [-@fig:006])
...
start и запустим её.
Так же посмотрим диассемблированный код (рис. [-@fig:007], [-@fig:008])
...
Выполним инструкций с помощью команды si и проследим за
...
Посмотрим значение переменной msg1 
Посмотрим значение переменной msg2
...
Выведете в различных форматах значение регистра edx. С помощью команды set изменим значение регистра ebx:(рис. [-@fig:013])
...
5. Скопируем файл lab9-2.asm. Создадим исполняемый файл.
Загрузим исполняемый файл в отладчик, указав аргументы.

Для начала установим точку остановки перед первой инструкцией в программе
и запустим ее.
...
Посмотрим остальные позиции стека (рис. [-@fig:015])
...
1. Преобразуем программу из самостоятельного задания 1, лабораторной работы №9 , реализовав вычисление значения функции f(x)
...
2. Скопируем из листинга программу, неверно рабочую. Проверим её с помощью отладчика GDB, определим ошибку и исправим ее.(рис. [-@fig:018] [-@fig:019] [-@fig:020] [-@fig:021])
...
Приобрели навыки написания программ с использованием подпрограмм.
Ознакомились с методами отладки при помощи GDB и его основными возможностями.
